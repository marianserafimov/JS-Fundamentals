function airforce(dataRows) {
    let airport = new Map()
    let townStatistics = new Map()
    let townPlanes = new Map()

    for (let dataRow of dataRows) {
        let [planeId, town, passengersCount, action]= dataRow.split(" ")
        ...
        if(action === "land"){
            ...
            townStatistics.get(town)[0] += Number(passengersCount)
            ...
            townStatistics.get(town)[1] += Number(passengersCount)
            townPlanes.get(town).add(planeId)
        }
    }
...
airforce([  "Boeing474 Madrid 300 land",
